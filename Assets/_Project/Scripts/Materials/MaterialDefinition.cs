...
namespace PhysSim.Materials
{
    ...
    /// Из него MaterialApplier выводит и визуал (Standard/URP-Lit), и физику
    /// (масса ρ·V, трение, упругость, Cd). Создаётся либо из встроенной таблицы
    /// (MaterialSpecs, рантайм), либо CSV-импортёром (SO-ассеты, editor).
    /// </summary>
    [CreateAssetMenu(
    ...
    public sealed class MaterialDefinition : ScriptableObject
    {
        ...
        [SerializeField] private float dragCoefficient = 1f;     // парусность Cd
        ...
        public MaterialCategory Category { get { return category; } }
        ...
        public Texture2D AlbedoMap { get { return albedoMap; } }
        public Texture2D NormalMap { get { return normalMap; } }
        public Vector2 Tiling { get { return tiling; } }

        /// <summary>
        /// Инициализация из спецификации БЕЗ ассета (рантайм-создание через CreateInstance).
        /// </summary>
        public void RuntimeSetup(MaterialSpec spec)
        {
            id = spec.Id;
            displayName = spec.DisplayName;
            category = spec.Category;
            density = Mathf.Max(1f, spec.Density);
            staticFriction = Mathf.Clamp(spec.StaticFriction, 0f, 1.5f);
            dynamicFriction = Mathf.Clamp(spec.DynamicFriction, 0f, 1.5f);
            restitution = Mathf.Clamp01(spec.Restitution);
            dragCoefficient = Mathf.Clamp(spec.DragCoefficient, 0.01f, 2.5f);
            color = spec.Color;
            metallic = Mathf.Clamp01(spec.Metallic);
            smoothness = Mathf.Clamp01(spec.Smoothness);
        }
